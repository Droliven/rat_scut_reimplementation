...
from .utils import panel_fillna, parse_time
...
class HistoryManager:
    ...
    def get_global_panel(self, start, end, period=300, features=('close',)):
        ...
        coins = self.select_coins(start=end - self.__volume_forward - self.__volume_average_days * 24*60*60,
        ...
        # panel = pd.Panel(items=features, major_axis=coins, minor_axis=time_index, dtype=np.float32) # 已经被弃用
        multi_indexs = pd.MultiIndex.from_product([features, coins])
        panel = pd.DataFrame(index=time_index, columns=multi_indexs)
            ...
            for row_number, coin in enumerate(coins):
                for feature in features:
                    ...
                    serial_data = pd.read_sql_query(sql, con=connection,
                                                    ...
                                                    index_col="date_norm")
                    # panel.loc[feature, coin, serial_data.index] = serial_data.squeeze() # [35088, 1]
