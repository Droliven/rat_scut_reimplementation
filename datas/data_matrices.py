...
@file    : data_matrices.py
@author  : levon
@contact : levondang@163.com
@ide     : PyCharm
@time    : 2022-05-03 11:39
'''
import numpy as np
import pandas as pd

from .history_manager import HistoryManager
from .utils import get_type_list, get_volume_forward, ReplayBuffer, parse_time

class DataMatrices:
    def __init__(self, start=parse_time("2016/01/01"), end=parse_time("2018/01/01"), period=1800, batch_size=128, volume_average_days=30, buffer_bias_ratio=5e-5,
                 market="poloniex", coin_filter=11, window_size=31, feature_number=4, test_portion=0.08,
                 portion_reversed=False, online=False, is_permed=False, database_path=""):
        """
        ...
        self.__history_manager = HistoryManager(coin_number=coin_filter, end=self.__end,
                                                ...
                                                volume_forward=volume_forward, online=online, database_path=self.database_path)
        if market == "poloniex":
    ...
    def get_submatrix(self, ind):
        # return self.__global_data.values[:, :, ind:ind + self._window_size + 1] # items=features, major_axis=coins, minor_axis=time_index
        return self.__global_data.values[ind:ind + self._window_size + 1, :].reshape(-1, len(self.__global_data.columns.levels[0]), len(self.__global_data.columns.levels[1])).transpose(1, 2, 0) # index=time_index, columns=(features, coins)

    def get_submatrix_test_online(self, ind_start, ind_end):
        # return self.__global_data.values[:, :, ind_start:ind_end]
        return self.__global_data.values[ind_start:ind_end, :].reshape(-1, len(self.__global_data.columns.levels[0]), len(self.__global_data.columns.levels[1])).transpose(1, 2, 0) # index=time_index, columns=(features, coins)
