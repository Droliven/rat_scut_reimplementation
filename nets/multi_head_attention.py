...
from .operations import attention_operation
...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask, padding_price_q, padding_price_k):
        ...
        local_weight_q_list = [torch.softmax(local_weight_q[:, :, i:i + 1, i:i + self.local_context_length], dim=-1) for i
                               in range(q_size2)]
        ...
        # [128,11,31,31+4]->[128,11,1,5*31]
        local_weight_k_list = [torch.softmax(local_weight_k[:, :, i:i + 1, i:i + self.local_context_length], dim=-1) for i
                               in range(key_size2)]
        ...
        x, self.attn = attention_operation(query, key, value, mask=None,
        ...
        if (self.asset_atten):
            ...
            x, self.attn_asset = attention_operation(ass_query, ass_key, ass_value, mask=None,
                                           dropout=self.dropout)
